...
# nnstreamer: examples/nnstreamer.cpp.filter/nnstreamer_llama_class.cpp  ggml.o llama.o $(COMMON_DEPS) console.o grammar-parser.o $(OBJS)
# 	$(CXX) $(CXXFLAGS) `pkg-config --cflags --libs nnstreamer-cpp` -shared -Wl,-soname,libnnstreamer-llama.so 
# 	examples/nnstreamer.cpp.filter/nnstreamer_llama_class.cpp $(filter-out %.h $<,$^) $(call GET_OBJ_FILE, $<) -o libnnstreamer-llama.so $(LDFLAGS) `pkg-config --cflags --libs nnstreamer-cpp`
